...
if __name__ == "__main__":
    ...
    line1, = ax.plot([], [], lw=2, marker='.', color='b', label='robot')
    line2, = ax.plot([], [], lw=2, marker='.', color='r', label='target')
    ...
    def update(frame):
        frame *= SPEEDUP
        if frame > len(robot_trajectory)-1:
            frame = len(robot_trajectory)-2
    ...
    ani = FuncAnimation(fig, update, frames=int(len(robot_trajectory)-1 / SPEEDUP), init_func=init, blit=False, interval=1)
